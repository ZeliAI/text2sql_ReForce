# adm_hk_asap_dwd_push_log_clk_di（push点击日志明细表）
...
| 属性 | 值 |
| --- | --- |
| 完整路径 | antsg_anthk_sg.adm_hk_asap_dwd_push_log_clk_di |
| 层级 | DWD（明细层） |
| 主键 | - (日志表无主键) |
| 外键 | delivery_push_task_id,creative_id → adm_hk_asap_dwd_push_task_info_dd |
| 分区 | dt (yyyyMMdd) |
| 说明 | push域点击日志明细表，用于分析push消息的点击转化效果 |
...
| user_id | string | 用户id(2088开头) | 208810217... | �� 高 |
...
| experiment_ids | string | ABTest的experimentIds | exp_12345_groupB | ⚡ 中 |
...
FROM adm_hk_asap_dwd_push_log_clk_di
WHERE dt = '${bizdate}'
GROUP BY dt
```
...
GROUP BY t.delivery_push_task_id
```
