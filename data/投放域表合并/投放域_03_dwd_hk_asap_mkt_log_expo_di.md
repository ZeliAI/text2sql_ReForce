...
| 属性 | 值 |
| --- | --- |
| 完整路径 | antsg_anthk_sg.dwd_hk_asap_mkt_log_expo_di |
| 层级 | DWD（明细层） |
| 主键 | - |
| 外键 | space_code, content_id → dws_hk_asap_mkt_distribute_index_1d |
| 分区 | dt (yyyyMMdd) |
| 说明 | 投放域曝光日志日增明细表，用于按用户/事件追踪曝光行为 |

> ⚠️ **选表说明**：本表为 DWD 明细层，包含每条曝光事件记录。仅在需要用户级明细（如 experiment_ids、utdid、log_type 等）或需关联明细维度时才使用。一般曝光统计请优先使用 DWS 表。
...
| experiment_ids | string | ABTest的实验id | exp_12345_groupB | 高 |
