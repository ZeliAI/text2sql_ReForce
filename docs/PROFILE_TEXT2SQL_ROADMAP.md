...
当前团队测试入口已经统一到仓库根目录：`run_online_agent.py`、`build_domain_registry.py`、`online_agent/`、`configs/`、`.env.example`、`requirements.txt`。
...
| V1.1 线上化工程重构 | Done | 隔离旧 Spider / ReFoRCE 榜单代码，新建线上 pipeline，支持节点级模型配置、路由、日期调整、安全兜底、mock online data 入口、长任务 timing / resume。 | `ONLINE_TEXT2SQL_REFACTOR_PLAN.md`、`online_agent/`、`run_online_agent.py`、`configs/online_agent_v1_2.json`、`data/mock_warehouse/` |
