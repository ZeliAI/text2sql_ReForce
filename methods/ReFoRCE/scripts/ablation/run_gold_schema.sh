...
"$PYTHON_BIN" spider_agent_setup_${TASK}.py --example_folder examples_${TASK}_gold_schema

# Reconstruct data
"$PYTHON_BIN" reconstruct_data.py \
    --example_folder examples_${TASK}_gold_schema \
    --add_description \
    --add_sample_rows \
    --make_folder \
    --clear_long_eg_des \
    --use_gold_schema \
    --gold_sql_pth ../../spider2-${TASK}/evaluation_suite/gold/sql

echo "Number of prompts.txt files in examples_${TASK}_gold_schema larger than 200KB before reducing: $(find examples_${TASK}_gold_schema -type f -name "prompts.txt" -exec du -b {} + | awk '$1 > 200000' | wc -l)"

# OUTPUT_PATH="output/${API}-${TASK}-log-${TIMESTAMP}"
OUTPUT_PATH="output/${API}-${TASK}-gold-schema-log"
NUM_VOTES=8
NUM_WORKERS=16
...
CMD1="\"$PYTHON_BIN\" run.py \
    --task $TASK \
    --db_path examples_${TASK}_gold_schema \
