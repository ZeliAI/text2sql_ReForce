...
"$PYTHON_BIN" spider_agent_setup_${TASK}.py --example_folder examples_${TASK}_debug

# Reconstruct data
"$PYTHON_BIN" reconstruct_data.py \
    --example_folder examples_${TASK}_debug \
    --add_description \
    --add_sample_rows \
    --rm_digits \
    --make_folder \
...
echo "Number of prompts.txt files in examples_${TASK}_debug larger than 200KB before reducing: $(find examples_${TASK}_debug -type f -name "prompts.txt" -exec du -b {} + | awk '$1 > 200000' | wc -l)"

# Run Schema linking and voting
"$PYTHON_BIN" schema_linking.py \
    --task $TASK \
    --db_path examples_${TASK}_debug \
