...
"$PYTHON_BIN" spider_agent_setup_${TASK}.py --example_folder examples_${TASK}_gold

# Reconstruct data
"$PYTHON_BIN" reconstruct_data.py \
    --example_folder examples_${TASK}_gold \
    --add_description \
    --add_sample_rows \
    --make_folder \
    --clear_long_eg_des \
...
echo "Number of prompts.txt files in examples_${TASK}_gold larger than 200KB before reducing: $(find examples_${TASK}_gold -type f -name "prompts.txt" -exec du -b {} + | awk '$1 > 200000' | wc -l)"

# OUTPUT_PATH="output/${API}-${TASK}-log-${TIMESTAMP}"
OUTPUT_PATH="output/${API}-${TASK}-gold-table-log"
NUM_VOTES=8
NUM_WORKERS=16
