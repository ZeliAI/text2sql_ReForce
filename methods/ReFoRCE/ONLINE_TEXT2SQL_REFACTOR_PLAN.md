...
当前 V1.1 已完成软隔离落地。新线上代码位于 `methods/ReFoRCE/online_agent/`，入口为 `methods/ReFoRCE/online_profile_agent.py`。V1.1 配置保留为 `methods/ReFoRCE/configs/online_agent_v1_1.json`；V1.2 默认配置为 `methods/ReFoRCE/configs/online_agent_v1_2.json`，在同一线上框架上扩展多域 registry 与 LLM router。

建议新建独立线上目录，避免继续受 Spider / ReFoRCE 榜单代码影响。

```text
methods/online_text2sql/
  configs/
  pipeline/
  nodes/
  llm/
...
```text
methods/legacy_reforce/
```

如果一次性移动风险较高，第一步可以先软隔离：

```text
methods/ReFoRCE/online_agent/
methods/ReFoRCE/legacy_spider/
```

但新代码不应再依赖 Spider2 / Snow / OmniSQL 的运行入口。
...
1. Done：新线上软隔离目录 `methods/ReFoRCE/online_agent/`。
...
5. Done：V1.0 画像域能力迁移到新框架，可通过 `online_profile_agent.py` 跑画像域样本。
